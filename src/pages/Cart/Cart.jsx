...
import { ShoppingCart, Plus, Minus, Trash2, ArrowLeft, Image, X, CheckCircle } from 'lucide-react';
...
export default function Cart() {
  ...
  // Temporizador de 10 segundos para nuevo pedido
  useEffect(() => {
    if (countdown > 0) {
      ...
      <div className="container">
        ...
        <div className="cart-content">
          ...
          <div className="cart-summary">
            ...
            <button className="checkout-btn" disabled={checkingOut || countdown > 0} onClick={async () => {
              setCheckingOut(true);
              let orderId = null;
              try {
              ...
              {countdown > 0 ? (
                <>
                  <span className="countdown-text">Esperar {countdown}s</span>
                </>
              ) : (
                <>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <p className="modal-message">Tu pedido ha sido enviado correctamente por WhatsApp</p>
            {lastOrderId && (
              <div className="modal-order-info">
                <span className="modal-order-label">Número de Pedido:</span>
                <span className="modal-order-id">{lastOrderId}</span>
              </div>
            )}
            <p className="modal-tracking-text">
              Puedes rastrear tu pedido usando este número en la página de seguimiento
            </p>
            <div className="modal-buttons">
              <Link to={`/pedido/${lastOrderId}`} className="modal-track-btn" onClick={() => setShowModal(false)}>
                Rastrear Pedido
              </Link>
              <button className="modal-continue-btn" onClick={() => setShowModal(false)}>
                Continuar Comprando
              </button>
            </div>
          </div>
