...
// DELETE /api/orders/:id — eliminar pedido (admin), requiere motivo
router.delete('/:id', authenticateToken, async (req, res) => {
  const { reason } = req.body;
  if (!reason?.trim()) {
    return res.status(400).json({ error: 'Debes ingresar un motivo de eliminación' });
  }
  try {
    const { rows } = await pool.query('SELECT id FROM orders WHERE id=$1', [req.params.id]);
    if (!rows.length) return res.status(404).json({ error: 'Pedido no encontrado' });
    await pool.query('DELETE FROM orders WHERE id=$1', [req.params.id]);
    res.json({ success: true, message: `Pedido #${req.params.id} eliminado. Motivo: ${reason}` });
  } catch (err) {
